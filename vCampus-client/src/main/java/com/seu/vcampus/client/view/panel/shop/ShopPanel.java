    ...
    private void addToCart() {
        ...
        SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
            ...
            protected Boolean doInBackground() throws Exception {
                return shopController.addToCart(currentUser.getCid(), productId, quantity);
            }
    ...
    private void purchaseItems() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
                ...
                protected Boolean doInBackground() throws Exception {
                    // 先添加到购物车
                    boolean added = shopController.addToCart(currentUser.getCid(), productId, quantity);
                    if (!added) return false;

                    // 然后创建订单
                    return shopController.createOrder(currentUser.getCid(), shippingAddress, contactPhone);
                }
        ...
        SwingWorker<Integer, Void> worker = new SwingWorker<Integer, Void>() {
            ...
            protected Integer doInBackground() throws Exception {
                List<CartItem> cartItems =
                        shopController.getCartItems(currentUser.getCid());
                int count = 0;
    ...
    public void clearCart() {
        if (currentUser != null) {
            SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
                @Override
                protected Boolean doInBackground() throws Exception {
                    return shopController.clearCart(currentUser.getCid());
                }
